...
def parse_args(args=None):
    ...
    parser.add_argument("file_out", type=Path, help="Output JSON file with selected data and additional fields.")
    parser.add_argument("-l", "--log-level", help="Log level (default: WARNING).", choices=("CRITICAL", "ERROR", "WARNING", "INFO", "DEBUG"), default="WARNING")
...
def parse_json_input(file_in):
    ...
    for row in data:
        ...
        if not experiment_accession:  # Check if 'experiment_accession' is missing or empty
            raise ValueError("Missing 'experiment_accession' in one or more items.")

        run_accession = row.get("run_accession")
        row_id = f"{experiment_accession}_{run_accession}" if run_accession else experiment_accession
        row["id"] = row_id
